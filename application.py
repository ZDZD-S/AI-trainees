    ...
    while True:
        ...
        if choice == MENU_CHOICE['Add job']:
            ...
            while current_status not in PROJECT_STATUS:
                print(f"Invalid status. Choose from: {', '.join(PROJECT_STATUS)}")
                current_status = input("Current status: ").strip()

            Job = job(title, department, job_type, description, requirements, salary_range, current_status)
            Job.save()
        # Process candidate entry with validation.
            ...
            while not (phone_number.isdigit() and len(phone_number) <= MAX_PHONE_NUMBER_LENGTH):
                print(INVALID_PHONE_NUMBER_ERROR.format(length=MAX_PHONE_NUMBER_LENGTH))
                phone_number = input("Phone Number: ").strip()
            ...
            Candidate = candidate(first_name, last_name, phone_number, email, address, linkedin_profile, education, experience, skills)
            Candidate.save()
